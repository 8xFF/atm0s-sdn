...
async fn run_server(sdk: VirtualSocketSdk, opts: ServerOpts) {
    ...
    while let Ok((mut stream, remote_addr)) = listener.accept().await {
        ...
        async_std::task::spawn(async move {
            ...
            match connector.connect_to(opts.dest, "TUNNEL_APP", HashMap::new()).await {
                Ok(socket_relay) => {
...
async fn run_agent(sdk: VirtualSocketSdk, opts: AgentOpts) {
    ...
    while let Some(socket) = listener.recv().await {
        log::info!("[TcpTunnel][Server] incomming conn from {:?}", socket.remote());
        let mut stream = VirtualStream::new(socket);
        async_std::task::spawn(async move {
            log::info!("[TcpTunnel][Server] connecting to target {}", opts.target);
            match TcpStream::connect(&opts.target).await {
                Ok(mut target) => {
                    log::info!("[TcpTunnel][Server] connected to target {}", opts.target);
                    let mut buf1 = [0; 4096];
                    let mut buf2 = [0; 4096];
                    loop {
                        select! {
                ...
                Err(e) => {
                    log::info!("[TcpTunnel][Server] connect to target {} error {:?}", opts.target, e);
                }
            }
        });
    }
...
async fn main() {
    ...
    async_std::task::spawn(async move {
        match args.mode {
            Mode::Server(opts) => run_server(virtual_socket_sdk, opts).await,
            Mode::Agent(opts) => run_agent(virtual_socket_sdk, opts).await,
        }
    });
