...
///
/// ```
/// let addr = p_8xff_sdn_multiaddr::from_url("ws://127.0.0.1:8080/").unwrap();
...
/// assert!(p_8xff_sdn_multiaddr::from_url(addr).is_err());
/// assert!(p_8xff_sdn_multiaddr::from_url_lossy(addr).is_ok());
/// ```
///
...
fn from_url_inner_http_ws(url: url::Url, lossy: bool) -> std::result::Result<Multiaddr, FromUrlErr> {
    ...
    if !lossy && (!url.username().is_empty() || url.password().is_some() || (lost_path && url.path() != "/" && !url.path().is_empty()) || url.query().is_some() || url.fragment().is_some()) {
