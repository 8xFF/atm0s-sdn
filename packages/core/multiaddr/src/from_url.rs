...
///
/// ```
/// let addr = atm0s_sdn_multiaddr::from_url("ws://127.0.0.1:8080/").unwrap();
...
/// assert!(atm0s_sdn_multiaddr::from_url(addr).is_err());
/// assert!(atm0s_sdn_multiaddr::from_url_lossy(addr).is_ok());
/// ```
///
