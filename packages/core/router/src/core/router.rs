...
impl Router {
    ...
    pub fn set_direct(&mut self, over: ConnId, over_node: NodeId, metric: Metric) {
        debug_assert_eq!(metric.hops(), &[over_node], "direct metric must has a single hops");
    ...
    pub fn apply_sync(&mut self, conn: ConnId, src: NodeId, metric: Metric, sync: RouterSync) {
        debug_assert_eq!(metric.hops(), &[src], "direct metric must has a single hops");
        self.service_registry.apply_sync(conn, src, metric.clone(), sync.0);
        for (index, table_sync) in sync.1.into_iter().enumerate() {
            if let Some(table_sync) = table_sync {
                self.tables[index].apply_sync(conn, src, metric.clone(), table_sync);
            }
        }
    }
...
mod tests {
    ...
    fn create_manual_multi_layers() {
        ...
        router.set_direct(z_node1_conn, z_node1, Metric::new(1, vec![z_node1], 1));
        router.set_direct(node1_conn, node1, Metric::new(1, vec![node1], 1));

        assert_eq!(router.size(), 2);

        //same zone, group
        assert_eq!(router.next(node1, &[]), Some((node1_conn, node1)));
        assert_eq!(router.next_path(node1, &[]), Some(Path::new(node1_conn, node1, Metric::new(1, vec![node1], 1))));
    ...
    fn conn(from: NodeId, to: NodeId) -> (ConnId, ConnId) {
        (ConnId::from_in(0, ((from as u64) << 32) | to as u64), ConnId::from_out(0, ((from as u64) << 32) | to as u64))
    }
    ...
    fn simple_relay_route() {
        ...
        let node1 = 1;
        let node3 = 3;
        let (node2, mut router2) = create_router(2);
        let (_conn12, conn21) = conn(node1, node2);

        router2.set_direct(conn21, node1, Metric::new(0, vec![node1], 0));
        assert_eq!(router2.tables[0].slots(), vec![node1.layer(0)]);

        router2.apply_sync(
            conn21,
            node1,
            Metric::new(0, vec![node1], 0),
            RouterSync(RegistrySync(vec![]), [Some(TableSync(vec![(node3.layer(0), Metric::new(0, vec![node3], 0))])), None, None, None]),
        );
        assert_eq!(router2.tables[0].slots(), vec![node1.layer(0), node3.layer(0)]);
    }
    ...
    fn complex_sync_same_zone() {
        ...
        let (conn_ad, conn_da) = conn(node_a, node_d);
        let (conn_ab, conn_ba) = conn(node_a, node_b);
        let (conn_bc, conn_cb) = conn(node_b, node_c);
        let (conn_be, conn_eb) = conn(node_b, node_e);
        let (conn_cf, conn_fc) = conn(node_c, node_f);
        let (conn_de, conn_ed) = conn(node_d, node_e);

        router_a.register_service(1);
        router_a.set_direct(conn_ad, node_d, Metric::new(1, vec![node_d], 1));
        router_a.set_direct(conn_ab, node_b, Metric::new(1, vec![node_b], 1));

        router_b.set_direct(conn_ba, node_a, Metric::new(1, vec![node_a], 1));
        router_b.set_direct(conn_bc, node_c, Metric::new(1, vec![node_c], 1));
        router_b.set_direct(conn_be, node_e, Metric::new(1, vec![node_e], 1));

        router_c.set_direct(conn_cb, node_b, Metric::new(1, vec![node_b], 1));
        router_c.set_direct(conn_cf, node_f, Metric::new(1, vec![node_f], 1));

        router_d.set_direct(conn_da, node_a, Metric::new(1, vec![node_a], 1));
        router_d.set_direct(conn_de, node_e, Metric::new(2, vec![node_e], 2));

        router_e.set_direct(conn_ed, node_d, Metric::new(2, vec![node_d], 2));
        router_e.set_direct(conn_eb, node_b, Metric::new(1, vec![node_b], 1));

        router_f.set_direct(conn_fc, node_c, Metric::new(1, vec![node_c], 1));
        ...
        assert_eq!(
            sync_a_b,
            RouterSync(
                RegistrySync(vec![(1, Metric::local())]),
                [
        ...
        for _i in 0..4 {
            ...
            router_b.apply_sync(conn_ba, node_a, Metric::new(1, vec![node_a], 1), sync_a_b);
            router_d.apply_sync(conn_da, node_a, Metric::new(1, vec![node_a], 1), sync_a_d);

            //for node B
            let sync_b_a = router_b.create_sync(node_a);
            let sync_b_c = router_b.create_sync(node_c);
            let sync_b_e = router_b.create_sync(node_e);
            router_a.apply_sync(conn_ab, node_b, Metric::new(1, vec![node_b], 1), sync_b_a);
            router_c.apply_sync(conn_cb, node_b, Metric::new(1, vec![node_b], 1), sync_b_c);
            router_e.apply_sync(conn_eb, node_b, Metric::new(1, vec![node_b], 1), sync_b_e);

            //for node C
            let sync_c_b = router_c.create_sync(node_b);
            let sync_c_f = router_c.create_sync(node_f);
            router_b.apply_sync(conn_bc, node_c, Metric::new(1, vec![node_c], 1), sync_c_b);
            router_f.apply_sync(conn_fc, node_c, Metric::new(1, vec![node_c], 1), sync_c_f);

            //for node D
            let sync_d_a = router_d.create_sync(node_a);
            let sync_d_e = router_d.create_sync(node_e);
            router_a.apply_sync(conn_ad, node_d, Metric::new(1, vec![node_d], 1), sync_d_a);
            router_e.apply_sync(conn_ed, node_d, Metric::new(2, vec![node_d], 2), sync_d_e);

            //for node E
            let sync_e_b = router_e.create_sync(node_b);
            let sync_e_d = router_e.create_sync(node_d);
            router_b.apply_sync(conn_be, node_e, Metric::new(1, vec![node_e], 1), sync_e_b);
            router_d.apply_sync(conn_de, node_e, Metric::new(2, vec![node_e], 2), sync_e_d);

            //for node F
            let sync_f_c = router_f.create_sync(node_c);
            router_c.apply_sync(conn_cf, node_f, Metric::new(1, vec![node_f], 1), sync_f_c);
        }
        ...
        assert_eq!(router_a.next_path(node_b, &[]), Some(Path::new(conn_ab, node_a, Metric::new(1, vec![node_b], 1))));
        assert_eq!(router_a.next_path(node_c, &[]), Some(Path::new(conn_ab, node_c, Metric::new(2, vec![node_c, node_b], 1))));
        assert_eq!(router_a.next_path(node_d, &[]), Some(Path::new(conn_ad, node_d, Metric::new(1, vec![node_d], 1))));
        assert_eq!(router_a.next_path(node_e, &[]), Some(Path::new(conn_ab, node_b, Metric::new(2, vec![node_e, node_b], 1))));
        assert_eq!(router_a.next_path(node_f, &[]), Some(Path::new(conn_ab, node_b, Metric::new(3, vec![node_f, node_c, node_b], 1))));
        assert_eq!(router_f.service_next(1, &[]), Some(ServiceDestination::Remote(conn_fc, node_c)));
        assert_eq!(router_c.service_next(1, &[]), Some(ServiceDestination::Remote(conn_cb, node_b)));
        assert_eq!(router_b.service_next(1, &[]), Some(ServiceDestination::Remote(conn_ba, node_a)));
        assert_eq!(router_e.service_next(1, &[]), Some(ServiceDestination::Remote(conn_eb, node_b)));
        assert_eq!(router_d.service_next(1, &[]), Some(ServiceDestination::Remote(conn_da, node_a)));
        ...
        router_a.del_direct(conn_ab);
        router_b.del_direct(conn_ba);
        ...
        assert_eq!(
            ...
            Some(Path::new(conn_ad, node_d, Metric::new(4, vec![node_b, node_e, node_d], 1))) //
        );
        assert_eq!(
            router_a.next_path(node_c, &[node_a]),
            Some(Path::new(conn_ad, node_d, Metric::new(5, vec![node_c, node_b, node_e, node_d], 1)))
        );
        assert_eq!(
            router_a.next_path(node_d, &[node_a]),
            Some(Path::new(conn_ad, node_d, Metric::new(1, vec![node_d], 1))) //
        );
        assert_eq!(
            router_a.next_path(node_e, &[node_a]),
            Some(Path::new(conn_ad, node_d, Metric::new(3, vec![node_e, node_d], 1))) //
        );
        assert_eq!(
            router_a.next_path(node_f, &[node_a]),
            Some(Path::new(conn_ad, node_d, Metric::new(6, vec![node_f, node_c, node_b, node_e, node_d], 1)))
        );
    }

    #[test]
    fn closest_node() {
        let (_node_a, mut router_a) = create_router(NodeId::build(0, 0, 0, 1));
        ...
        router_a.set_direct(conn_5000, node_5000, Metric::new(1, vec![node_5000], 1));
        router_a.set_direct(conn_0500, node_0500, Metric::new(1, vec![node_0500], 1));
    ...
    fn closest_node_out_of_space() {
        let (_node_a, mut router_a) = create_router(NodeId::build(1, 0, 0, 1));
        ...
        router_a.set_direct(conn_0002, node_0002, Metric::new(1, vec![node_0002], 1));
        router_a.set_direct(conn_0003, node_0003, Metric::new(1, vec![node_0003], 1));
    ...
    fn to_key_consistency() {
        for _ in 0..100 {
            let (node_a, mut router_a) = create_router(rand::random());
            let (node_b, mut router_b) = create_router(rand::random());
