...
impl Router {
    ...
    pub fn set_direct(&mut self, over: NodeId, metric: Metric) {
        let eq_util_layer = self.local_node_id.eq_util_layer(&over) as usize;
        log::debug!(
    ...
    pub fn next_path(&self, dest: NodeId, excepts: &Vec<NodeId>) -> Option<Path> {
        ...
        if eq_util_layer == 0 {
            Some(Path(
                self.local_node_id,
                Metric::new(0, vec![self.local_node_id], 0),
            ))
        } else {
    ...
    pub fn apply_sync(&mut self, src: NodeId, src_send_metric: Metric, sync: RouterSync) {
        self.service_registry
            .apply_sync(src, src_send_metric.clone(), sync.0);
    ...
    fn complex_sync_same_zone() {
        ...
        **/
        ...
        assert_eq!(
            ...
            Some(Path(node_B, Metric::new(1, vec![node_B, node_A], 1)))
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(Path(node_D, Metric::new(1, vec![node_D, node_A], 1)))
        );
        assert_eq!(
