...
mod tests {
    ...
    async fn simple_network() {
        ...
        let tran1 = UdpTransport::new(1, 10001, node_addr_builder1.clone()).await;

        let node_addr_builder2 = Arc::new(NodeAddrBuilder::default());
        let tran2 = UdpTransport::new(2, 10002, node_addr_builder2.clone()).await;
    ...
    async fn simple_network_connect_addr_not_found() {
        let node_addr_builder1 = Arc::new(NodeAddrBuilder::default());
        let tran1 = UdpTransport::new(1, 20001, node_addr_builder1.clone()).await;
        network::transport_tests::simple::simple_network_connect_addr_not_found(tran1, NodeAddr::from_iter(vec![Protocol::Ip4(Ipv4Addr::new(127, 0, 0, 1)), Protocol::Udp(20002)])).await;
    }

    #[async_std::test]
    async fn simple_network_connect_wrong_node() {
        let node_addr_builder1 = Arc::new(NodeAddrBuilder::default());
        let tran1 = UdpTransport::new(1, 30001, node_addr_builder1.clone()).await;

        let node_addr_builder2 = Arc::new(NodeAddrBuilder::default());
        let tran2 = UdpTransport::new(2, 30002, node_addr_builder2.clone()).await;
